...
const useExtend = () => {
  ...
  const extendLayer = useMemo(() => {
    const layer = grenserLayers.measure;
    layer.setStyle(grenseStyles.measure);
    return layer;
  }, []);

  const extendInteraction = useMemo(() => {
    return new Draw({
      source: extendLayer.getSource()!,
      type: "LineString",
  ...
  useEffect(() => {
    if (activeTool === "measure") {
      ...
      const onDrawStart = (evt: DrawEvent) => {
        extendLayer.getSource()?.clear(); // Nullstill forrige måling/opptegning
      ...
      extendInteraction.on("drawstart", onDrawStart);
      extendInteraction.on("drawend", onDrawEnd);

      // Cleanup
      return () => {
        isDrawing.current = false;
        extendInteraction.un("drawstart", onDrawStart);
        extendInteraction.un("drawend", onDrawEnd);
        // Fjerner eventuell oppmåling/tegning
        extendLayer.getSource()?.clear();
  ...
  }, [activeTool, extendInteraction, extendLayer]);

  return { extendInteraction };
};
