...
const useGetMuligeKretserForNyAdministrativGrense = (
  ...
  const { data: grunnkretserIfExists } = useGrunnkretser(grunnkretsEgenskapIDer.filter((id) => id.length > 0));
  const { data: stemmekretserIfExists } = useStemmekretser(stemmekretsEgenskapIDer.filter((id) => id.length > 0));
    ...
    switch (kontekstType) {
      case KontekstType.GRUNNKRETS: {
        return {
          ...
          [Tilhorighet.B]: grunnkretserAsTilhorighetKrets!,
        };
      }
      case KontekstType.STEMMEKRETS: {
        return {
          [Tilhorighet.A]: mapStemmekretRefToKrets(stemmekretser),
          [Tilhorighet.B]: stemmekretserAsTilhorighetKrets!,
        };
      }
    }
