...
export const highlightFormatter = (option: OptionType, formatOptionLabelMeta: FormatOptionLabelMeta<OptionType>) => {
...
export const TilhorighetSearch = ({ options, value, onChange }: Props) => {
  const loadResults = async (term: string, resultsCallback: (options: OptionType[]) => void) => {
    ...
    if (term == null || term === "") {
      ...
      resultsCallback(options.filter((option) => option.label.includes(term)).slice(0, 40));
    }
  };
